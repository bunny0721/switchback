...
def test_window_length_must_be_positive_integer():
    with pytest.raises(ValueError):
        BernoulliDesign(window_length=0)
    with pytest.raises(ValueError):
        BernoulliDesign(window_length=-1)
...
def test_consecutive_prob_within_one_window():
    ...
    design = BernoulliDesign(window_length=4, p=0.3)
...
def test_consecutive_prob_three_windows():
    design = BernoulliDesign(window_length=2, p=0.5)
...
def test_window_marginal_probability_via_monte_carlo():
    ...
    design = BernoulliDesign(window_length=3, p=0.4, seed=0)
...
def test_seed_reproducibility():
    d1 = BernoulliDesign(window_length=3, seed=42)
    d2 = BernoulliDesign(window_length=3, seed=42)
    np.testing.assert_array_equal(d1.sample(12), d2.sample(12))


def test_reset_replays_same_path():
    d = BernoulliDesign(window_length=2, seed=7)
    a = d.sample(10)
    d.reset()
    b = d.sample(10)
...
def test_complete_rand_assignment_constant_within_window():
    design = CompleteRandomization(window_length=3, seed=0)
    T = 24  # 8 windows (even)
    for _ in range(50):
        W = design.sample(T)
        for b in range(8):
...
def test_complete_rand_window_index_partitions_horizon():
    design = CompleteRandomization(window_length=3)
    np.testing.assert_array_equal(
...
def test_complete_rand_n_windows_must_be_even():
    """n_windows = T/window_length must be even (for the half/half split)."""
...
def test_complete_rand_window_length_must_be_positive_integer():
    with pytest.raises(ValueError):
        CompleteRandomization(window_length=0)
    with pytest.raises(ValueError):
        CompleteRandomization(window_length=-1)


def test_complete_rand_seed_reproducibility():
    a = CompleteRandomization(window_length=2, seed=42).sample(20)
    b = CompleteRandomization(window_length=2, seed=42).sample(20)
    np.testing.assert_array_equal(a, b)
...
def test_complete_rand_prob_zero_when_more_than_half_windows():
    ...
    design = CompleteRandomization(window_length=1)
...
def test_complete_rand_converges_to_bernoulli_for_large_n():
    ...
    n = 1000
    T = n * K
    design = CompleteRandomization(window_length=K)
